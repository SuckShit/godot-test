...
@onready var heart_container: Node2D = $HudLayer/HeartContainer
@onready var heart_texture: Texture2D = preload("res://resources/texture/heart.png")
...
@onready var win_bgm_player: AudioStreamPlayer = $AudioContainer/WinPlayer
@onready var loss_bgm_player: AudioStreamPlayer = $AudioContainer/LossPlayer
@onready var time_left_label: Label = $HudLayer/TimeLeftLabel
...
func _setup_hud() -> void:
	...
	time_left_label.text = str(stage_time_left)
	...
	var heart_gap := 1  # 心形之间的间距
	for i in range(max_hp):
		var heart := Sprite2D.new()
		heart.texture = heart_texture
		heart.position = Vector2(i * (heart_texture.get_width() + heart_gap), 0)
...
func _update_hud() -> void:
	...
	_update_time_left_label()
	_update_time_bar()

# 根据当前生命值更新心形精灵的可见性
func _update_life_cnt_label() -> void:
	var current_hp := _get_player_current_health()
	for i in range(heart_sprites.size()):
		heart_sprites[i].visible = i < current_hp

func _update_time_left_label() -> void:
	time_left_label.text = "%d" % stage_time_left
...
# 结束前先停止播放背景音乐，再根据结局播放胜利或失败音乐
func _play_result_bgm(title_name: String) -> void:
	if bgm_player.playing:
		bgm_player.stop()

	if title_name == RESULT_TITLE_WIN:
		_play_sfx(win_bgm_player)
		return
	
	if title_name == RESULT_TITLE_LOSE:
		_play_sfx(loss_bgm_player)
